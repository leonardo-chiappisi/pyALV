...
CONTINPath = os.path.join('contin', './contin-linux')
...
def runCONTINfit(xdata, ydata, parameterFile, continInputFile=None, continOutputFile=None):
    ...
    if continInputFile == None:
        continInputFile = os.path.join('contin',"CONTINInput.txt")
        # print("continInputFile", continInputFile)
    if continOutputFile == None:
        continOutputFile = os.path.join('contin',"CONTINOutput.txt")
...
def readCONTINoutput(filename):
    ...
    fit_func_start = re.compile('0PLOT')
    fit_func_end = re.compile('0ERROR')
    # print(fit_func)
    ...
    with open(filename, 'r') as fileObj:
        for i, line in enumerate(fileObj):
            if fit_func_start.search(line) is not None:  # Or whatever test is needed
                print(i)    
            # break
        
        # for line in fileObj:
            # if fit_func_end.search(line) is not None:
                # break
